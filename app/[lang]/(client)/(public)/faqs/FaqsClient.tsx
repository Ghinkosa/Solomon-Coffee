...
const FaqsClient = ({ dictionary }: FaqsClientProps) => {
  const toLocalizedPath = useLocalizedPath();
  const page = dictionary?.faqsPage ?? {};
        ...
        <Container className="max-w-6xl">
          ...
          >
            ...
            <p className="text-xl text-white/90 max-w-3xl mx-auto mb-6">
              {hero.description ??
                "Quick answers to the most commonly asked questions about Sheba's Coffee."}
            </p>
            <Badge className="bg-white/20 text-white border-white/30 hover:bg-white/30">
              {hero.badge ?? "Essential Information"}
            </Badge>
          </motion.div>
        </Container>
      </section>
          ...
          <Card className="mb-8">
            <CardHeader>
              <CardTitle className="text-center text-shop_dark_green">
                {content.title ?? "Common Questions & Answers"}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <Accordion
                ...
                defaultValue="item-0"
              >
                {questions.map((faq: { question: string; answer: string }, index: number) => (
                  <motion.div
                    key={index}
