  ...
  const handlePayNow = async () => {
    ...
    try {
      ...
      if (data.success && data.checkoutUrl) {
        toast.success(p("toasts.redirecting", "Redirecting to secure payment..."));
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : p("toasts.paymentFailed", "Payment failed");
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogPortal>
        ...
        >
          <VisuallyHidden.Root>
            <DialogTitle>{p("title", "Secure Payment")}</DialogTitle>
          </VisuallyHidden.Root>
          ...
          <div className="text-center space-y-4">
            ...
            <div className="space-y-2">
              <h3 className="text-2xl font-bold text-shop_dark_green flex items-center justify-center gap-2">
                {p("title", "Secure Payment")}
                <Sparkles className="w-5 h-5 text-shop_orange animate-pulse" />
              </h3>
              <p className="text-gray-600 leading-relaxed">
                {p("subtitle", "Complete your payment securely with Stripe")}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center gap-2 p-4 bg-shop_dark_green/5 rounded-lg border border-shop_dark_green/10 animate-fade-in">
            ...
            <span className="text-sm text-shop_dark_green font-semibold">
              {p("sslProtection", "Protected by 256-bit SSL encryption")}
            </span>
          </div>
          ...
          <div className="flex items-center justify-center gap-3 text-xs text-gray-500 animate-fade-in">
            <div className="flex flex-col items-center gap-1">
              ...
              <span className="font-medium">{p("stepSecure", "Secure")}</span>
            </div>
            ...
            <div className="flex flex-col items-center gap-1">
              ...
              <span className="font-medium">{p("stepPay", "Pay")}</span>
            </div>
            <div className="w-8 h-0.5 bg-shop_dark_green/30"></div>
            <div className="flex flex-col items-center gap-1">
              ...
              <span className="font-medium">{p("stepComplete", "Complete")}</span>
            </div>
          </div>
            ...
            >
              {isProcessing ? (
                <div className="flex items-center justify-center gap-2">
                  ...
                  <span>{p("processing", "Processing...")}</span>
                </div>
              ) : (
                <div className="flex items-center justify-center gap-2">
                  <CreditCard className="w-5 h-5" />
                  <span>{p("paySecurely", "Pay Securely Now")}</span>
                </div>
              )}
            </Button>
          </div>
          ...
          <div className="flex items-center justify-center gap-6 text-xs text-gray-400 border-t border-gray-100 pt-4 animate-fade-in">
            <div className="flex items-center gap-1">
              <Lock className="h-3.5 w-3.5 text-shop_light_green" />
              <span>{p("sslSecured", "SSL Secured")}</span>
            </div>
            <div className="flex items-center gap-1">
              <Zap className="h-3.5 w-3.5 text-shop_orange" />
              <span>{p("instant", "Instant")}</span>
            </div>
            <div className="flex items-center gap-1">
              <ShieldCheck className="h-3.5 w-3.5 text-shop_dark_green" />
              <span>{p("protected", "Protected")}</span>
            </div>
          </div>
